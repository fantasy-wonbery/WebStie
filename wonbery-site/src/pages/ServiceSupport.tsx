...
const systems: SystemDef[] = [
  { key: 'msp', icon: Cloud, image: 'images/pages/improve-operational-efficiency-header.jpg' },
  { key: 'hybrid', icon: Server, image: 'images/pages/service17.jpg' },
]
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative pt-16 pb-20 bg-dark overflow-hidden">
        <div className="absolute inset-0 opacity-20 bg-cover bg-center" style={{ backgroundImage: `url(${B}images/pages/improve-operational-efficiency-header.jpg)` }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-primary/20 border border-primary/30 text-primary text-sm font-medium mb-6">
            {t('serviceSupport.badge')}
          </div>
          <h1 className="text-4xl md:text-5xl font-display font-bold text-white mb-4">{t('serviceSupport.title')}</h1>
          <p className="text-white/60 text-lg max-w-2xl mx-auto mb-8">{t('serviceSupport.subtitle')}</p>
      ...
      <section className="py-20">
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-8">
            {systems.map((sys, i) => {
              const Icon = sys.icon
              const features = t(`serviceSupport.systems.${sys.key}.features`, { returnObjects: true })
              return (
                <motion.div key={sys.key} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                  ...
                  <div className="p-6">
                    <h3 className="text-lg font-display font-bold text-gray-900 mb-2 group-hover:text-primary transition-colors">{t(`serviceSupport.systems.${sys.key}.title`)}</h3>
                    <p className="text-gray-500 text-sm leading-relaxed mb-4">{t(`serviceSupport.systems.${sys.key}.description`)}</p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setModal(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="relative h-64 md:h-80"><img src={B + modal.image} alt="" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-6 left-6"><h2 className="text-2xl md:text-3xl font-display font-bold text-white">{t(`serviceSupport.systems.${modal.key}.title`)}</h2></div>
              </div>
              <div className="p-6 md:p-8">
                <p className="text-gray-700 leading-relaxed mb-6">{t(`serviceSupport.systems.${modal.key}.description`)}</p>
                {(() => { const f = t(`serviceSupport.systems.${modal.key}.features`, { returnObjects: true }); return Array.isArray(f) ? (
